...
def parse_rules(text: str):
    """'원본=수정' 줄들 → [(a,b)]. 글자 수가 달라도 됨. '원본=삭제'는 그 단어를 지움(나머지 위치 유지)."""
    rules = []
...
def render_page(img: Image.Image, rows, new_texts, dpi: int = DPI, preview: bool = False, scale: float = None,
    ...
    for ri, (row, new) in enumerate(zip(rows, new_texts)):
        ...
        if len(new) != len(old):
            ...
            try:
                notes_cache = note_xs(np.array(img.convert("L")), row, s)
            except Exception:
                notes_cache = None
            ...
            for tag, a1, a2, b1, b2 in difflib.SequenceMatcher(None, old, new, autojunk=False).get_opcodes():
                ...
                if seg:
                    ...
                    targets = [ex0 + step * (k + 0.5) for k in range(n_)]
                    # 박자 정렬: 구간 안의 음표 머리 x에 스냅 (개수가 맞을 때)
                    if notes_cache is not None:
                        free = [x for x in notes_cache
                                if ex0 - pitch * 0.4 <= x <= ex1 + pitch * 0.4]
                        if len(free) >= n_:
                            targets = free[:n_]
...
def note_xs(gray, row, s):
    """가사 행 위 오선 구간에서 음표 머리의 x중심들을 찾는다 (박자 정렬용). 실패하면 []"""
    sylls = row.get("sylls")
    if not sylls:
        return []
    top = min(b[1] for b in sylls)
    y1 = max(0, int(top - 10 * s)); y0 = max(0, int(top - 170 * s))
    x0 = max(0, min(b[0] for b in sylls) - int(60 * s))
    x1 = min(gray.shape[1], max(b[2] for b in sylls) + int(60 * s))
    if y1 - y0 < 30 * s:
        return []
    band = (gray[y0:y1, x0:x1] < 128).astype(np.uint8)
    # 오선 제거
    k = cv2.getStructuringElement(cv2.MORPH_RECT, (int(40 * s), 1))
    band = cv2.subtract(band, cv2.morphologyEx(band, cv2.MORPH_OPEN, k))
    n, _, st_, ce = cv2.connectedComponentsWithStats(band, connectivity=8)
    xs = []
    for i in range(1, n):
        x, y, w, h, area = st_[i]
        if 8 * s <= w <= 26 * s and 7 * s <= h <= 20 * s and area / (w * h + 1e-6) > 0.55:
            xs.append(x0 + x + w / 2)
    xs.sort()
    # 너무 가까운 중복(화음) 병합
    merged = []
    for x in xs:
        if merged and x - merged[-1] < 12 * s:
